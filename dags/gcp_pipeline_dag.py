...
PROJECT_ID = Variable.get("GCP_PROJECT_ID", default_var="stunning-prism-485817-v9")
SUBSCRIPTION_ID = "sales-sub"
BUCKET_NAME = Variable.get("GCS_BUCKET_NAME", default_var="datapath-sales-bucket-stunning-prism-485817-v9")
...
with DAG(
    ...
    def process_and_upload_to_gcs(ti):
        ...
        if not pulled_messages:
            raise AirflowSkipException("No messages found from Pub/Sub.")
        ...
        if not processed_records:
            raise AirflowSkipException("No valid records found.")
    ...
    create_dataset >> pull_messages >> process_task >> load_to_bq >> transform_bq
